...
public class ChannelStrip {
    ...
    private final List<JNAEffect> offline = new ArrayList<>();

    // All effects known to this channel (RT + offline + LFOs etc.)
    protected final List<JNAEffect> effects = new ArrayList<>();

    // fx activate/deactivate flag
    private volatile boolean activeDirty = false;

    protected ChannelStrip() {
        pendingActive.addAll(active);
    }

    /** Effects ready at creation */
    public ChannelStrip(JNAEffect... bus) {
        this();
        for (JNAEffect fx : bus) {
            effects.add(fx);
    ...
    public void toggle(JNAEffect effect) {
    ...
    public /* final */ void reset() {
        ...
        for (JNAEffect fx : offline) {
            fx.reset();
        }
        offline.clear();

        // reset all effect internals (regardless of whether they were active)
        for (JNAEffect fx : effects) {
            fx.reset();
        }
        // gui updates left to subclasses
    }

    public void setActive(JNAEffect fx, boolean on) {
        boolean currentlyOn = isActive(fx);
        if (on == currentlyOn) return;
        toggle(fx);
    }

    public boolean isActive(JNAEffect effect) {
        if (rt.contains(effect))
            return pendingActive.contains(effect);
        return offline.contains(effect);
    }

    public List<JNAEffect> listAll() {
    	return new ArrayList<JNAEffect>(effects);
    }
