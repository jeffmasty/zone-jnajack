...
import judahzone.jnajack.fx.JackBiquad.FilterType;
import judahzone.util.Constants;
import lombok.Getter;

public class JackFilter implements RTEffect {
    ...
    @Getter private final String name = JackFilter.class.getSimpleName();
    @Getter private final int paramCount = Settings.values().length;

    private final JackBiquad filter;
...
//	float hz = type == FilterType.HighPass ? 100 : 4000;
	...
	@Override public int get(int idx) {
		if (idx == Settings.dB.ordinal())
			...
			return (int) (filter.bandwidth * 100 / JackBiquad.MAX_WIDTH);
