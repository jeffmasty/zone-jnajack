...
public class JackEQ implements RTEffect {
	...
	public static enum Settings{ Bass, Mid, High, LoHz, Q, HiHz }
 	...
 	public static final int MIN_HZ = JackFilter.MIN;
 	public static final int MAX_HZ = JackFilter.MAX;
	...
	@Getter private final String name = JackEQ.class.getSimpleName();
	@Getter private final int paramCount = Settings.values().length;
	private final ArrayList<JackBiquad> stereo = new ArrayList<JackBiquad>();
	...
	public JackEQ(Frequencies hz) {
		stereo.add(new JackBiquad(hz.low));
		stereo.add(new JackBiquad(hz.mid));
		stereo.add(new JackBiquad(hz.high));
	}

	private void update(JackBiquad filter, Properties param, float value) {
		switch (param) {
	...
	public void eqGain(EqBand eqBand, int val) {
        update(eqBand, Properties.dB, JackBiquad.gainDb(val));
	}
