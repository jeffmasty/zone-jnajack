...
public class JackChorus implements JNATime, RTEffect {
    ...
    @Override public String getName() {
        return JackChorus.class.getSimpleName();
    }
    ...
    private class LFODelay {
        ...
        public void processAdd(FloatBuffer buf) {
            ...
            for (int i = 0; i < N_FRAMES; i++) {
                ...
                float inSample = buf.get(i);
                float fbSample = o * feedback;
                fbFilterState = fbFilterState + fbCut * (fbSample - fbFilterState);
                // overwrite with filtered feedback contribution
                workArea[pos] = inSample + fbFilterState;

                buf.put(i, inSample + o);
                pos = (pos + 1) % rnlen;
                ldelay += delta;
            }
            rovepos = pos;
